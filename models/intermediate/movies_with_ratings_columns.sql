 WITH ratings_rows AS (
    ...
    FROM
        ...
        GROUP BY identifier_unique_key, title, ratings, flat_outer_array.INDEX
...
ratings_columns AS (
    ...
    PIVOT (MAX(RATING_VALUE) FOR RATING_SOURCE IN ('Internet Movie Database', 'Rotten Tomatoes', 'Metacritic')) 
        AS p (identifier_unique_key, title, ratings, imdb_rating, rotten_tomatoes_rating, metacritic_rating)
)
