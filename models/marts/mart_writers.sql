select u.writer
     ...
     , m.metascore 
     , m.imdb_rating
     , avg(m.profit)      over (partition by u.writer order by m.release_date rows between 2 preceding and current row)   as threefilm_avg_profit
     , avg(m.imdb_rating) over (partition by u.writer order by m.release_date rows between 2 preceding and current row)   as threefilm_avg_imdb_rating
